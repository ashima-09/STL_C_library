...
//Prototypes of functions used in implementing vector like structure.
...
void merge_sort(int *vector, long long int l, long long int r);
...
//Prototypes of functions used in implementing Set like structure.
...
void vector_sort(struct vector*vector)
{
    merge_sort(vector->vector, 0, vector->size-1);
}
...
void set_erase(set t, int key) // deletes a node
{
  node *z= find(t,key);
  node *y = z;
  node *x;
...
int main()
{
    ...
    *vector_at(v, 2)=5;
    ...
    printf("Printing values in Set a : ");
      ...
      printf("After inserting duplicate element: \n");
      printf("Printing values in Set a : ");
      display(a,a->root);
      printf("\n");
      printf("Size of a : %d\n",size(a));
